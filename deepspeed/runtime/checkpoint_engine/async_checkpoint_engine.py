...
class AsyncCheckpointEngine(CheckpointEngine):
    ...
    def _background_persist(self):
        ...
        while len(self.snapshots) > 0:
            ...
            # logger.info(f"[AsyncTorch] In _background_persist for path {path}")
            t = time.time()
            torch.save(state_dict, path)
            # logger.info(f"[AsyncTorch] Time to complete background persist {time.time()-t} for path {path}")
    ...
    def _background_snapshot(self, state_dict, path):
        ...
        #     self.checkpoint_in_progress = False
        #     self.in_progress_cv.notify_all()
        logger.info(f"[AsyncTorch] Time to complete background snapshot {time.time()-t} for path {path}")
    ...
    def commit(self, tag):
        ...
        logger.info(f"[AsyncTorch] Checkpoint {tag} is ready now!, persist futures {len(self.persist_futures)}")
        return True
    ...
    def wait(self, prev_version = -1):
        ...
        logger.info(f"[AsyncTorch] Wait time in checkpointing engine {time.time()-t}")   
        return True
