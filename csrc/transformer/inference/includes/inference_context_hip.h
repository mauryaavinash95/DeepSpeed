...
public:
          ...
          _workSpaceSize(0)
    {
        ...
        rocblas_status stat = rocblas_create_handle(&_cublasHandle);
        if (stat != rocblas_status_success) {
            // It would be nice to use cublasGetStatusName and
            // cublasGetStatusString, but they were only added in CUDA 11.4.2.
            auto message = std::string("Failed to create cublas handle: rocblas_status was ") +
                           std::to_string(stat);
    ...
    bool retake_workspace()
    {
        ...
        hipMalloc(&_workspace, _workSpaceSize);
